...
- **Diagram rendering** — `render_graph` emits a `Graph` to one format through `graph_to_mermaid` / `graph_to_dot` / `graph_to_json`. This is the single point any diagram is produced from; rendering is deterministic, never model-generated.
