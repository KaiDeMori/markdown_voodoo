...
- **Diagram rendering** — `conversation_graph` reduces a fork family to the render-neutral `Graph` (the structured tree result, what `tree --format json` serialises); `render_graph` then emits that `Graph` to one text dialect through `graph_to_mermaid` / `graph_to_dot`. `render_conversation_tree` is the convenience that does both. Rendering is deterministic, never model-generated. There is no `graph_json` dialect: a graph as JSON is the universal `--format json` over the `Graph` dataclass, not a diagram format.
